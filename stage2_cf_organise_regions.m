...
region_counter=region_counter+1;meta.regions.name(region_counter)={'EU27'}; %1
region_counter=region_counter+1;meta.regions.name(region_counter)={'EU15'}; %2
region_counter=region_counter+1;meta.regions.name(region_counter)={'OECD'}; %3
region_counter=region_counter+1;meta.regions.name(region_counter)={'BRICS'}; %4
region_counter=region_counter+1;meta.regions.name(region_counter)={'Annex I'}; %5
region_counter=region_counter+1;meta.regions.name(region_counter)={'G8'}; %6
region_counter=region_counter+1;meta.regions.name(region_counter)={'EU12'}; %7
region_counter=region_counter+1;meta.regions.name(region_counter)={'North America'}; %8
region_counter=region_counter+1;meta.regions.name(region_counter)={'EU28'}; %9
region_counter=region_counter+1;meta.regions.name(region_counter)={'Global'}; %10
%include individual countries common to ICIO
meta.regions.name=[meta.regions.name,pbca_emis.ICIO.ISO3(1:end-1)'];
meta.regions.unique_country=ones(size(meta.regions.name,2),1);
meta.regions.unique_country(1:region_counter)=0;
meta.regions.unique_country=logical(meta.regions.unique_country);



n_regions=size(meta.regions.name,2)
...
for region_count=1:n_regions
    ...
    regionname_orig=meta.regions.name{region_count};
    ...
    dash_indx=strfind(regionname,'&');
    regionname(dash_indx)='';
    meta.regions.regionname_clean{region_count}=regionname;
    clear dash_indx
    ...
    if strcmp('Global',meta.regions.name(region_count))
        indx_of_reg_poptab=1+strcmp('WLD',poptab.CountryCode);
        regions_match_indx_pop=2*ones(length(indx_of_reg_poptab),1);
        indx_of_reg_ppptab=1+strcmp('WLD',ppptab.CountryCode);
        regions_match_indx_ppp=2*ones(length(indx_of_reg_ppptab),1);
    end
    ...
    for modelcounter=1:meta.number_models
        ...
        raw_regional_results(modelcounter,region_count).regionmodname=[meta.modelname{modelcounter},'_',regionname];
    ...
    for modelcounter=1:meta.number_models
        ...
        if strcmp('Global',meta.regions.name(region_count))
            %global emissions - only GCP has meta.regions in it which prevent
            %summing over all.
            if strcmp(meta.modelname(modelcounter),'GCP')
                raw_regional_results(modelcounter,region_count).reg.pbca(1:size(pbca_emis.(meta.modelname{modelcounter}),2)-2,1)=sum(pbca_emis.(meta.modelname{modelcounter}){end,3:end},1)';
                raw_regional_results(modelcounter,region_count).reg.cbca(1:size(cbca_emis.(meta.modelname{modelcounter}),2)-2,1)=sum(cbca_emis.(meta.modelname{modelcounter}){end,3:end},1)';
            else
                raw_regional_results(modelcounter,region_count).reg.pbca(1:size(pbca_emis.(meta.modelname{modelcounter}),2)-2,1)=sum(pbca_emis.(meta.modelname{modelcounter}){:,3:end},1)';
                raw_regional_results(modelcounter,region_count).reg.cbca(1:size(cbca_emis.(meta.modelname{modelcounter}),2)-2,1)=sum(cbca_emis.(meta.modelname{modelcounter}){:,3:end},1)';
            end
            raw_regional_results(modelcounter,region_count).reg.pbca(raw_regional_results(modelcounter,region_count).reg.pbca==0)=NaN;
            raw_regional_results(modelcounter,region_count).reg.cbca(raw_regional_results(modelcounter,region_count).reg.cbca==0)=NaN;
        else
        
        
        %production account
        [indx_of_reg_pbca,regions_match_pbca,~]=regions_and_countries(pbca_emis.(meta.modelname{modelcounter}).ISO3,regionname_orig,'regionmembership.csv');
        reg_match_indx_pbca=find(strcmp(regionname_orig,regions_match_pbca));
        %consumption account:
        [indx_of_reg_cbca,regions_match_cbca,~]=regions_and_countries(cbca_emis.(meta.modelname{modelcounter}).ISO3,regionname_orig,'regionmembership.csv');
        ...
        [reg_matches,~,~]=intersect(pbca_emis.(meta.modelname{modelcounter}).ISO3(indx_of_reg_pbca==reg_match_indx_pbca),ppptab.CountryCode(indx_of_reg_ppptab==regions_match_indx_ppp));
        ...
        [reg_matches,~,~]=intersect(cbca_emis.(meta.modelname{modelcounter}).ISO3(indx_of_reg_cbca==reg_match_indx_cbca),ppptab.CountryCode(indx_of_reg_ppptab==regions_match_indx_ppp));
        [~,~,matchvals]=intersect(reg_matches,ppptab.CountryCode);
        ...
        missing_countries=setxor(cbca_emis.(meta.modelname{modelcounter}).ISO3(indx_of_reg_cbca==reg_match_indx_cbca),ppptab.CountryCode(indx_of_reg_ppptab==regions_match_indx_ppp))';
        if ~isempty(missing_countries)
            ...
            if length(indx_of_reg_cbca)>1
                disp([meta.modelname{modelcounter} ,' missing these countries from the regional aggregation: ', strjoin(missing_countries(:))])
                disp(['     Percent missing in gdp: ', num2str(100*(1-mean(tmp_mod_macro.gdpt_percent))),'%'])
            else
                
            disp([meta.modelname{modelcounter} ,' missing this country :',missing_countries{:},''])
            end
        end
...
%         tmp_emis=nansum(pbca_emis.(meta.modelname{modelcounter}){indx_of_reg_pbca==reg_match_indx_pbca,3:end},1)';
        tmp_emis=pbca_emis.(meta.modelname{modelcounter}){indx_of_reg_pbca==reg_match_indx_pbca,3:end};
        tmp_emis_nan=pbca_emis.(meta.modelname{modelcounter}){indx_of_reg_pbca==reg_match_indx_pbca,3:end};
...
%       find years that have NaN values:
        ...
        tmp=nansum(cbca_emis.(meta.modelname{modelcounter}){indx_of_reg_cbca==reg_match_indx_cbca,3:end},1)';
