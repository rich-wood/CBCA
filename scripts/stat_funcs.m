...
for i=1:n_models
    abs_diff(:,i)=nansum(abs(datain(region_count).all(:,i)-datain(region_count).all(:,setxor(i,[1:n_models]))),2)./datain(region_count).n;
end
datain(region_count).mean_diff=nansum(abs_diff,2)./datain(region_count).n;
